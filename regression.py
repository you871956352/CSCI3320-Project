...
test_raw_data = test_data.merge(temp_j, left_on='jockey_index', right_on='jockey_index', how='left')
...
test_raw_data = test_data.merge(temp_t, left_on='trainer_index', right_on='trainer_index', how='left')
...
def gbrt_model(X, y):
    start_time = time.time()
    gbrt_model1 = GBRT(loss='ls')
    gbrt_model2 = GBRT(loss='lad')
    gbrt_model3 = GBRT(loss='huber')
    gbrt_model4 = GBRT(loss='quantile')
    ls = gbrt_model1.fit(X, y).score(X, y)
    lad = gbrt_model1.fit(X, y).score(X, y)
    huber = gbrt_model1.fit(X, y).score(X, y)
    quantile = gbrt_model1.fit(X, y).score(X, y)
    print(ls,lad,huber,quantile)
    # Run time
    running_time = time.time() - start_time
    print("Running time : %s seconds" % running_time)
...
def evaluation(result,truth):
    ...
    while (i < len(result)):
        if (race_id != test_raw_data['race_id'][i]):
            ...
            race_sum += 1
            top_id = i
        else if (result[i] < result[top_id]):
                top_id = i
        # Next sample
        i += 1
